...
#if defined(NDEBUG)
# error "TokenOfHope cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5da7b2a085d20dca009edc0a060a0bc34cb0b62d523efa90a6c3811d2093e6c9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TokenOfHope: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TokenOfHope Signed Message:\n";
...
{
    // TokenOfHope: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TokenOfHope: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TokenOfHope: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1530745233, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "2018Token Of Hope Will Rule The W0rld2020";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0.012 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048651f410b298d5ee5d2ffa03f99736db84b64579c20fab59620ed3496ada448509399d9c6b21467f696879d61b6af800a30170091bd266555849bf3e70a8f8cd") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1530745233;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3312083;
        ...
        assert(block.hashMerkleRoot == uint256("0xb874a1e5b01e1b32f54aa087ed003533d57b07e97c3e750e35a402491028d2a0"));
...
unsigned char pchMessageStart[4] = { 0x01, 0xd7, 0x1d, 0xb2 }; // TokenOfHope: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TokenOfHopeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TokenOfHopeMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TokenOfHopeMiner(CWallet *pwallet)
{
    printf("TokenOfHopeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tokenofhope-miner");
    ...
    try { loop {
        ...
        printf("Running TokenOfHopeMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TokenOfHopeMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TokenOfHopeMiner, pwallet));
}
